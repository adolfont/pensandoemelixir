...
propagate_flash(Map, Lines, Columns) ->
  ...
  update_map_with_flashes(Flashes, Map, 0, Lines, Columns).

update_map_with_flashes([], Map, FlashCounter, _Lines, _Columns) ->
  {FlashCounter, Map};
...
update_neighbors([], Map, Lines, Columns) ->
  {_, NewMap} = propagate_flash(Map, Lines, Columns),
  NewMap;
